...
use AwesomeCoder\Utils\Base;
use AwesomeCoder\Utils\Http;
use AwesomeCoder\Utils\Plan;
...
abstract class API extends Base {
	...
	private static function is_allowed( $type, $class ){
		...
		if( ! class_exists( '\\AwesomeCoder\\'. ucfirst( $type ) .'\\' . ucfirst( $class ) ) ) {
			return false;
		}
		return true;
	}
	...
	protected function call_user_func( $type, $parameters = [] ) {
		...
		if( ! empty ( $parameters[0] ) && self::is_allowed( $type, $parameters[0] ) ) {
			return call_user_func( [ '\\AwesomeCoder\\'. ucfirst( $type ) .'\\' . ucfirst( $parameters[0] ), 'get_instance' ] );
		}
	...
	protected function get_namespace( $endpoint = '' ) {
		return '/' . EEA_API_NAMESPACE . ( ! empty( $endpoint ) ? "/$endpoint" : '' );
	}
	...
	protected function permission_error( $message, $endpoint = '') {
        if( empty( $message ) ) {
            $message = __( 'Sorry, you need to login/re-login again.', 'AwesomeCoder' );
        }
