...
use AwesomeCoder\Interface\Jsonable;
use AwesomeCoder\Support\Collection;
use AwesomeCoder\Support\Enumerable;
use AwesomeCoder\Interface\Arrayable;
use AwesomeCoder\Traits\Conditionable;
...
trait EnumeratesValues
{
    ...
    public function value($key, $default = null)
    {
        if ($value = $this->firstWhere($key)) {
            return Util::data_get($value, $key, $default);
        }
    ...
    public function whereIn($key, $values, $strict = false)
    {
        ...
        return $this->filter(fn ($item) => in_array(Util::data_get($item, $key), $values, $strict));
    }
    ...
    {
        return $this->filter(
            fn ($item) => Util::data_get($item, $key) < reset($values) || Util::data_get($item, $key) > end($values)
        );
    }
    ...
    public function whereNotIn($key, $values, $strict = false)
    {
        ...
        return $this->reject(fn ($item) => in_array(Util::data_get($item, $key), $values, $strict));
    }
        ...
        foreach ($this as $key => $value) {
            ...
            if (!is_array($result)) {
                throw new UnexpectedValueException(sprintf(
                    ...
                    Util::class_basename(static::class),
                    gettype($result)
                ));
            }
        }
    ...
    protected function operatorForWhere($key, $operator = null, $value = null)
    {
        ...
        return function ($item) use ($key, $operator, $value) {
            $retrieved = Util::data_get($item, $key);
    ...
    protected function valueRetriever($value)
    {
        ...
        return fn ($item) => Util::data_get($item, $value);
    }
