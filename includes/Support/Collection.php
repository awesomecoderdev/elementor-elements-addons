...
class Collection implements ArrayAccess, CanBeEscapedWhenCastToString, Enumerable
{
    ...
    {
        if (func_num_args() === 2) {
            return $this->contains(fn ($item) => Util::data_get($item, $key) === $value);
        }
     ...
     *
     * \AwesomeCoder\Contracts\Support\Arrayable<array-key, TValue>|iterable<array-key, TKey>|TKey  $keys
     *
     * @return $this
     */
    ...
    public function getOrPut($key, $value)
    {
        ...
        $this->offsetSet($key, $value = Util::value($value));
        ...
        uasort($items, function ($a, $b) use ($comparisons) {
            foreach ($comparisons as $comparison) {
                ...
                if (!is_string($prop) && is_callable($prop)) {
                    ...
                    $values = [Util::data_get($a, $prop), Util::data_get($b, $prop)];
