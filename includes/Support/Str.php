...
class Str
{
    ...
    public static function excerpt($text, $phrase = '', $options = [])
    {
        ...
        $start = static::str(mb_substr($start, max(mb_strlen($start, 'UTF-8') - $radius, 0), $radius, 'UTF-8'))->ltrim()->unless(
        ...
        $end = static::str(mb_substr($end, 0, $radius, 'UTF-8'))->rtrim()->unless(
     ...
     * @return bool
     */
    function json_validate($jsonString, $options = 0) {
        $jsonData = json_decode($jsonString, $options);
    ...
    public static function isUrl($value, array $protocols = [])
    {
        ...
        return preg_match(str_replace('EEA_PROTOCOLS', $protocolList, $pattern), $value) > 0;
    }
