...
class Http extends Base {
     ...
     * @var string
     */
    private $url = 'https://api.awesomecoder.dev/eea';
    ...
    public function __construct() {
        $this->dev_mode = defined( 'EEA_DEVELOPMENT_MODE' ) && EEA_DEVELOPMENT_MODE;
    }
    ...
    public function url() {
        if ( $this->dev_mode ) {
            $this->url = 'https://api.awesomecoder.dev/eea';
        }
        return $this->url;
    }
    ...
    public function post( $args = [] ) {
        ...
        $headers = [
            ...
            'x-AwesomeCoder-ip'  => Helper::get_ip(),
            'x-AwesomeCoder-url' => home_url( '/' )
        ];
        ...
        if ( defined( 'EEA_DEBUG_LOG' ) && EEA_DEBUG_LOG ) {
                    ...
                    if ( isset( $error['message'] ) ) {
                        if ( $error['message'] === 'validation' ) {
                            ...
                            if ( isset( $error['debugMessage'] ) ) {
                                $wp_error->add( 'eea_error', $error['debugMessage'] );
                            } else {
                                $wp_error->add( 'eea_error', $error['message'] );
                            }
                        }
                    }
                } );

				if( $wp_error->get_error_code() === 'eea_error' ) {
